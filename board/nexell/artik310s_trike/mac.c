...
void generate_mac(int index)
{
	...
	/* Sanity check */
	if (index >= MAX_CODES || index < 0)
		return;

	snprintf(ethname, 16, index ? "eth%daddr" : "ethaddr", index);

	/* Check ethernet mac address */
	ethaddr = env_get(ethname);
	if (ethaddr != NULL)
		return;
	...
	mac = SET_MODEL_FIELD(info->model_code[index]);

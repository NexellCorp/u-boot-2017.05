...
static int eqos_adjust_link(struct udevice *dev)
{
	...
	if (en_calibration) {
		if (eqos->drv_ops && eqos->drv_ops->calibrate) {
			ret = eqos->drv_ops->calibrate(dev);
			if (ret < 0) {
		...
		if (eqos->drv_ops && eqos->drv_ops->disable_calibrate)
...
static int eqos_start(struct udevice *dev)
{
	...
	if (eqos->drv_ops && eqos->drv_ops->start) {
		ret = eqos->drv_ops->start(dev);
		if (ret < 0) {
			pr_err("eqos_start failed: %d", ret);
			goto err;
		}
	}
	...
	if (eqos->drv_ops && eqos->drv_ops->get_tick_clk_rate) {
		rate = eqos->drv_ops->get_tick_clk_rate(dev);
	} else {
		pr_err("get_tick_clk_rate should be provided");
		ret = -ENODEV;
		goto err_stop_resets;
	}
	val = (rate / 1000000) - 1;
...
err_shutdown_phy:
	...
	eqos->phy = NULL;
err_stop_resets:
	if (eqos->drv_ops && eqos->drv_ops->stop)
		eqos->drv_ops->stop(dev);
err:
	pr_err("FAILED: %d", ret);
	return ret;
}

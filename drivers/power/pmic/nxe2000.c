...
{
	ofnode node, chg_node;
	int children;

	debug("%s: dev->name:%s\n", __func__, dev->name);

	if (!strncmp(dev->name, "nxe2000", 7)) {
		node = dev_read_subnode(dev, "voltage-regulators");
		chg_node = dev_read_subnode(dev, "init-charger");
	} else {
		node = dev_read_subnode(dev, "voltage-regulators");
	}
	...
	if (node.np > 0) {
		...
		children = pmic_bind_children(dev, node, pmic_reg_info);
		if (!children)

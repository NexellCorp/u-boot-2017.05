...
static int sm5011_ldo_get_enable(struct udevice *dev)
{
	const struct sm5011_para *ldo_param = NULL;
	int ldo = dev->driver_data;
...
static int sm5011_ldo_ofdata_to_platdata(struct udevice *dev)
{
	...
	pdata->init_uV = dev_read_u32_default(dev, "sm5011,init_uV", -ENODATA);
	debug(" pdata->init_uV=%d\n", pdata->init_uV);

	pdata->boot_on = dev_read_bool(dev, "regulator-boot-on");
	debug(" pdata->boot_on=%d\n", pdata->boot_on);

	pdata->always_on = dev_read_bool(dev, "regulator-always-on");
	debug(" pdata->always_on=%d\n", pdata->always_on);

	pdata->on = dev_read_u32_default(dev, "sm5011,on", -ENODATA);
	debug(" pdata->on=%d\n", pdata->on);
...
static int sm5011_buck_get_enable(struct udevice *dev)
{
	int buck = dev->driver_data;

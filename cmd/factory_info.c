...
#ifndef CONFIG_FACTORY_INFO_BUF_ADDR
...
static int fi_mmc_ops(int state, int dev, int part, u32 blk, u32 cnt,
		      void *addr)
{
	u32 n;
	struct mmc *mmc;
	...
	ret = blk_select_hwpart_devnum(IF_TYPE_MMC, dev, original_part);
	if (ret)
		return CMD_RET_FAILURE;

	return (n == cnt) ? 0 : CMD_RET_FAILURE;
}
...
static int factory_info_load(const char *interface, const char *device,
			     const char *partition, const char *offset,
			     const char *count)
{
	int err;
	...
	if (!strncmp(interface, "mmc", 3)) {
		...
		int part = simple_strtoul(partition, NULL, 10);
		u32 blk = simple_strtoul(offset, NULL, 16);
		u32 cnt = simple_strtoul(count, NULL, 16);
		err = fi_mmc_ops(LOAD, dev, part, blk, cnt,
			     ...
			     const char *count)
{
	...
	if (!strncmp(interface, "mmc", 3)) {
		...
		u32 blk = simple_strtoul(offset, NULL, 16);
		u32 cnt = simple_strtoul(count, NULL, 16);
		err = fi_mmc_ops(SAVE, dev, part, blk, cnt,
				(void *)CONFIG_FACTORY_INFO_BUF_ADDR);
		if (err)
			return err;
	}
...
U_BOOT_CMD(factory_info, 7, 0, do_factory_info,
	   "Factory Information commands",
	   "list - List factory information\n"
	   "factory_info load <interface> <device> <hwpart> <offset> <cnt>\n"
	   "        - Load factory information from the interface\n"
	   "factory_info save <interface> <device> <hwpart> <offset> <cnt>\n"
